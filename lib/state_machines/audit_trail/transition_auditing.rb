  ...
  module AuditTrail
    module TransitionAuditing
      ...
      def audit_trail(options = {})
        state_machine = self
        if options[:class].presence && !options[:class].is_a?(Class)
          raise ":class option[#{options[:class]}] must be a class (not a string)."
        end
          ...
          state_machine.owner_class.after_initialize do |object|
            if state_machine.backend.new_record? object
              ...
              unless current_state.nil?
                state_machine.backend.log(object,
                                          InitialTransition.new(namespace: state_machine.namespace,
                                                                to: current_state))
              end
            end
          end
